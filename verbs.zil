...
<ROUTINE V-COLOR ("AUX" (DEFAULT <>))
	 <COND (<APPLE?>
		<TELL
"[So sorry -- you can't change your text and background colors on this system.]" CR>
		<RTRUE>)
	       (<NOT ,COLOR-NOTE>
		<SETG COLOR-NOTE T>
		<TELL
...
<ROUTINE V-CREDITS ()
	 ...
	 <CENTER-3 "Tanya Allan" "Denise Audette" "Andy Briggs">
	 <CENTER-3 "Rob Dunlavey" "Sophia Green" "Joy Pulver">
	 <CENTER-1 "Charlie Voner">
	 <CRLF>
...
<ROUTINE V-COUGH ()
	 <COND ;(<AND <EQUAL? ,HERE ,ICKY-CAVE>	;"moved to -F"
...
<ROUTINE V-DIG ()
	 <COND (<PRSI? ,SHOVEL>
		<COND (<NOT <PRSO? ,GROUND>>
		      ...
		      ;(<AND <EQUAL? ,JUMP-X ,STUMP-X>	;"moved to OUTER-BAILEY-F"
...
of the hole collapse, filling it in, and you barely make it out alive." CR>)
		      (T
		       <TELL "You dig a sizable hole but, finding">
...
<ROUTINE V-LEAP ()
	 <COND (<AND ,TURNED-INTO
		...
		<COND (<AND <EQUAL? ,HERE ,GONDOLA>
		       ...
		       <DO-WALK ,P?OUT>)
		      ;(<EQUAL? ,HERE ,HANGING-FROM-ROOTS	;"moved to -F"
	 ...
	 <COND (<FSET? ,PRSO ,LOCKEDBIT>
		<TELL ,ALREADY-IS>)
	       (<OR <FSET? ,PRSO ,DOORBIT>
		    <PRSO? ,TRUNK>>
		<PRINT "Unfortunately,">
		<TELL T ,PRSI " doesn't lock" TR ,PRSO>)
	       (T
...
<ROUTINE V-LOOK-UNDER ()
	 <COND (<AND <ULTIMATELY-IN? ,PRSO>
		...
		<COND (<FSET? ,PRSO ,WORNBIT>
		       <TELL ;,WEARING-IT "You're wearing it!" CR>)
		      (T
		       <TELL ,HOLDING-IT>)>)
	       (<FSET? ,PRSO ,WATERBIT>
		<PERFORM-PRSA ,WATER>)
...
<ROUTINE V-SMELL ()
	 <COND (<NOT ,PRSO>
		<COND ;(<EQUAL? ,HERE ,WEST-WING>	;"moved to -F"
		       <TELL ,FUDGE CR>)
	 ...
	 <COND (<NOT .OBJ>
		...
		<RTRUE>)
	       (<AND <ULTIMATELY-IN? .OBJ ,YACHT>
		     <IN? ,PROTAGONIST ,HERE>>
		<RTRUE>)
	       (<AND <EQUAL? .OBJ ,PSEUDO-OBJECT>
		     <EQUAL? ,LAST-PSEUDO-LOC <LOC ,PLAYER>>>
		<RFALSE>)	
	       (<IN? ,PROTAGONIST ,HERE>
		<RFALSE>)
	       (<AND <EQUAL? .OBJ ,YACHT ,LAKE-FLATHEAD ,WEST-DOCK
				  ,EAST-DOCK ,NORTH-DOCK ,SOUTH-DOCK>
